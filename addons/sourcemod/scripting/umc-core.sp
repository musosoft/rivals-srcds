...
public Action:Command_SetNextmap(client, args)
{
    if (args < 1)
    {
        CReplyToCommand(
            client,
            "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Usage: sm_setnextmap <map> <0|1|2>\n 0 - Change Now\n 1 - Change at end of round\n 2 - Change at end of map."
        );
        return Plugin_Handled;
    }
    ...
    if (!IsMapValid(map))
    {
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Map '%s' was not found.", map);
        return Plugin_Handled;
    }
...
public Action:Command_Reload(client, args)
{
    ...
    CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} UMC Mapcycles Reloaded.");    
...
public Action:Command_StopVote(client, args)
{
    ...
    if (!stopped)
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} No map vote running!"); //TODO Translation?
    return Plugin_Handled;
}
    ...
    if (NeedRunoff(vM))
    {
        ...
        if (remaining_runoffs == 0 || prev_vote_count == 2)
        {
            ...
            else if (stored_fail_action == RunoffFailAction_Nothing)
            {
                ...
                CPrintToChatAll(
                    "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t (%t)",
                    "Vote Failed",
                    "Vote Win Percentage",
                        percentage,
                        total_votes
...
{    
   ...
   /*  //Play the vote start sound if...
    ...
    if (GetConVarBool(cvar_runoff_selective))
    {
        ...
        {
            CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Selective Runoff");
            vote_active = true;
        }
    }
                            ...
                            Float:percentage)
{
    ...
    {
        CPrintToChatAll(
            "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t %t (%t)",
...
{   
    ...
    switch (when)
    {
        ...
        case ChangeMapTime_RoundEnd: //We change the map at the end of the round.
        {
            ...
            //Print a message.
            CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Map Change at Round End");
        }
    }
}
