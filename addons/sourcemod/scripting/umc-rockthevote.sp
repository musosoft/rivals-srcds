...
public OnClientDisconnect_Post(client)
{
    ...
    {
        CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Player Disconnect RTV");
        StartRTV();
    }
}
...
AttemptRTV(client)
{
    ...
    if (!ClientHasAdminFlags(client, flags))
    {
        CPrintToChat(
            client,
            "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t",
            "No RTV Admin"
        );
        return;
    }
    ...
    if (rtv_completed || (vote_completed && GetConVarInt(cvar_rtv_postaction) == 1))
    {
        ...
        CPrintToChat(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "No RTV Nextmap");
        return;
    }
    ...
    {
        CPrintToChat(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "No RTV Time", rtv_delaystart);
        return;
    }
    ...
    else if (FindValueInArray(rtv_clients, client) == -1)
    {
        ...
        if (size >= rtv_threshold)
        {
            ...
            {
                CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "RTV Start");
                StartRTV();
            }
            else //Otherwise, display a message.
            {
                CPrintToChat(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Vote In Progress");
