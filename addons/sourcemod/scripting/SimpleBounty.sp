...
#define PLUGIN_VERSION "1.2.1-private"
...
public OnPluginStart()
{
	...
	g_version = CreateConVar("sm_simple_bounty_version", PLUGIN_VERSION, "Bounty Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
		...
		if(IsValidKill(client, attacker))
		{
			...
			if(ClientKills[attacker] == GetConVarInt(g_BountyKills))
			{
				...
				if(GetConVarInt(g_BountyDisplay) == 1)
					CPrintToChatAll("{chartreuse}[{dodgerblue}Bounty{chartreuse}]{default} %s has a bounty", AttackerName);
			...
			{
				if(GetConVarInt(g_BountyDisplay) == 1)
					CPrintToChatAll("{chartreuse}[{dodgerblue}Bounty{chartreuse}]{default} %s has taken %s\'s bounty of $%d", AttackerName, ClientName, ClientBounty[client]);
				else if(GetConVarInt(g_BountyDisplay) == 2)
...
public Action:EventRoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && ClientHasBounty[i] && IsPlayerAlive(i))
		{
			...
			if(GetConVarInt(g_BountyDisplay) == 1)
				CPrintToChat(i, "{chartreuse}[{dodgerblue}Bounty{chartreuse}]{default} You have an active bounty of $%d", ClientBounty[i]);
			else if(GetConVarInt(g_BountyDisplay) == 2)
...
public Action:SetBounty(client, args)
{
	...
	if(GetConVarInt(g_BountyDisplay) == 1)
		CPrintToChatAll("{chartreuse}[{dodgerblue}Bounty{chartreuse}]{default} %s had a bounty set on them for $%i", targetName, ammount);
	else if(GetConVarInt(g_BountyDisplay) == 2)
