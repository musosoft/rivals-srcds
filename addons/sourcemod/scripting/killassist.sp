...
#define PLUGIN_VERSION "1.6.0-morecolors"
...
public OnPluginStart()
{
	...
	if (StrEqual(szBuffer, "cstrike", false))
		g_currentMod = Working_Mod:GAMEMOD_CSS;
	else if (StrEqual(szBuffer, "csgo", false))
		g_currentMod = Working_Mod:GAMEMOD_CSGO;
	else if (StrEqual(szBuffer, "dod", false))
		g_currentMod = Working_Mod:GAMEMOD_DODS;
	else
		g_currentMod = Working_Mod:GAMEMOD_ELSE;
	
	//CVARs
	CreateConVar("killassistversion", PLUGIN_VERSION, "Kill Assist version", FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	
	g_hEnabled				= CreateConVar("kassist", "1.0", "If kill assist is enabled. 1=Yes, Def. 1", 0, true, 0.0, true, 1.0);
	g_hMinDmg				= CreateConVar("kassist_minDmg", "35.0", "Minimum damage required to assist a kill. Def. 35", 0, true, 1.0);
	g_hAddKillerScore		= CreateConVar("kassist_killValue", "0.0", "Bonus points/frags given to a killer. Integer. Def. 0 (disabled)", 0, true, 0.0);
	g_hAddAssisterScore		= CreateConVar("kassist_assistValue", "0.0", "Bonus points/frags given to an assister. Real. Def. 0.0 (disabled)", 0, true, 0.0);
	g_hAddRankMeScore		= CreateConVar("kassist_rankMeValue", "0.0", 
		"RankMe points given to an assister. Integer. Def. 0 (disabled). Only use other values if you have RankMe.", 0, true, 0.0);
	g_hVerbose				= CreateConVar("kassist_verbose", "2.0", "The default value of the verbose cookie. Tell players when they assist someone. 1=Yes (always), 2=Yes (when dead, def.).", 0, true, 0.0, true, 2.0);
	g_hVerboseCookie		= CreateConVar("kassist_verbose_cookie", "1.0", "If verbose cookies are used. 0=No, 1=Yes (Def.), 2=Yes (remove the 'always' option).", 0, true, 0.0, true, 2.0);
	...
	//Conditional CVars
	if ( g_currentMod == GAMEMOD_CSS || g_currentMod == GAMEMOD_CSGO )
	{
		g_hSplit			= CreateConVar("kassist_split", "1.0", "If kill assist cash is split amongst assisters. 1=Yes, Def. 1.", 0, true, 0.0, true, 1.0);
	...
	if ( g_currentMod == GAMEMOD_CSS )
	{
		g_hReward			= CreateConVar("kassist_cash", "150.0", "Kill assist cash awarded to assisters. Def. 150.", 0, true, 0.0);
		g_hUseMVPStars		= CreateConVar("kassist_useMVPStars", "2.0", 
		"Override MVPs (Stars) system to show assists ? 1=Yes (SDKHooks), 2=Yes (CS_SetMVPCount if available, else SDKHooks), Def. 2.", 0, true, 0.0, true, 2.0);
	}
	else if ( g_currentMod == GAMEMOD_CSGO )
	{
	...
	if ( g_currentMod != GAMEMOD_DODS ) //CSS, CSGO, possibly other
	{
	...
	if ( g_currentMod == GAMEMOD_CSS || g_currentMod == GAMEMOD_CSGO ) //CSS, CSGO
	{
		g_iAccountOffsets	= FindSendPropInfo("CCSPlayer", "m_iAccount");
		
		if ( g_currentMod == GAMEMOD_CSS )
		{
			g_iMVPs		= FindSendPropInfo("CCSPlayerResource", "m_iMVPs"); //klausenbusk ftw :D
...
public Event_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	if ( GetConVarInt(g_hEnabled) == 1 )
	{
		...
		if ( IsClientInGame( victimId ) )
		{
			...
			{
				if ( g_currentMod == GAMEMOD_CSS || g_currentMod == GAMEMOD_CSGO ) //Money = CS:S/GO
				{
					decl moneyToGive;
					
					if ( g_currentMod == GAMEMOD_CSS || 
							( g_currentMod == GAMEMOD_CSGO && GetConVarInt( g_hRewardType ) == 0 ) )
					{
...
public OnMapStart()
{
	if (g_currentMod == GAMEMOD_CSS && GetConVarInt( g_hUseMVPStars ) == 1 )
	{
...
Action:GiveMoney(assisters[], any:nbAssisters, any:cash, victimTeam)
{
	...
	new maxMoney = g_currentMod != GAMEMOD_CSGO ? 16000 : GetConVarInt( g_hConVarMaxMoney );
...
Action:GiveAssistPoints(assisters[], any:nbAssisters, any:killerId, victimId, csgoAssisterId/*to prevent adding assists*/, victimTeam )
{
	...
	for (new i; i < nbAssisters; ++i)
	{
		...
		if ( victimTeam != GetClientTeam( assisterId ) ) //the opposite should mostly happen in CSGO (or massive team swap stuff)
		{
			...
			else if (g_currentMod == GAMEMOD_CSGO) //allow many assiter per kill
			{
		...
		{
			if(GetClientTeam(assisterId) == CS_TEAM_CT)
			{
				CPrintToChat(assisterId, "{chartreuse}[{dodgerblue}Assist{chartreuse}]{DEFAULT} %t", "Assist", "{blue}", killerId, "{default}", "{red}", victimId, "{default}");
			}
			else {
				CPrintToChat(assisterId, "{chartreuse}[{dodgerblue}Assist{chartreuse}]{DEFAULT} %t", "Assist", "{red}", killerId, "{default}", "{blue}", victimId, "{default}");
			}
			//if ( g_bHasPrintedOnceThisMap[ assisterId ] == false && verboseCookie != 0 && verboseOncePerMap == true )
			//{
			//	CPrintToChatEx(assisterId, assisterId, "{chartreuse}[{dodgerblue}Assist{chartreuse}]{DEFAULT} %t", "TellHowToChange", "{teamcolor}", "{default}", "{teamcolor}", "{default}");
