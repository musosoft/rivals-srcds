...
#define KNIFEFIGHT_VERSION      "1.3.8c-private"
...
#define YELLOW                  {DEFAULT}
#define TEAMCOLOR               {TEAMCOLOR}
#define GREEN                   {GREEN}
...
{
    name = "Knife Fight",
    author = "XARiUS, Otstrel.Ru Team, muso.sk",
...
new bool:g_slaylosers;
new bool:g_slaylosersenabled     = false;
...
new Handle:g_Cvarslaylosers     = INVALID_HANDLE;
...
new Handle:slaylosers_enabled       = INVALID_HANDLE;
new g_WeaponParent;
...
{
    CreateNative( "KnifeFight_IsFighting", Native_IsFighting );
    return APLRes_Success;
}

public Native_IsFighting(Handle:plugin, numParams)
{
   return isFighting;
}
...
public OnPluginStart()
{
    ...
    new Handle:Cvar_Version = CreateConVar("sm_knifefight_version", KNIFEFIGHT_VERSION, 
        "Knife Fight Version.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
    ...
    g_Cvarslaylosers    = CreateConVar("sm_knifefight_slaylosers",     "0", 
        "Enable losing team slay (disable slaylosers_enabled) if slaylosers_enabled is enabled while knife fight. 0 = Disabled");
    ...
    HookConVarChange(g_Cvarslaylosers,          OnSettingChanged);
...
public OnConfigsExecuted()
{
    ...
    g_slaylosers        = GetConVarBool(g_Cvarslaylosers);
...
public OnSettingChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
    ...
    else if (convar == g_Cvarslaylosers)            g_slaylosers        = (newValue[0] == '1');
...
{    
    ...
    if ( g_slaylosers )
    {
        if ( slaylosers_enabled == INVALID_HANDLE )
        {
            slaylosers_enabled = FindConVar("slaylosers_enabled");
        }
        if ( slaylosers_enabled != INVALID_HANDLE )
        {
            g_slaylosersenabled = !GetConVarBool(slaylosers_enabled);
            if ( !g_slaylosersenabled )
            {
                SetConVarInt(slaylosers_enabled, 0);
            }
            g_slaylosersenabled = !g_slaylosersenabled;
        }
    }
...
public Action:Countdown(Handle:timer)
{
    ...
    SetEntProp(ctid, Prop_Data, "m_CollisionGroup", 17);
...
public EventRoundStart(Handle:event, const String:name[],bool:dontBroadcast)
{
    ...
    if ( g_slaylosers && g_slaylosersenabled )
    {
        g_slaylosersenabled = false;
        if ( slaylosers_enabled == INVALID_HANDLE )
        {
            slaylosers_enabled = FindConVar("slaylosers_enabled");
        }
        if ( slaylosers_enabled != INVALID_HANDLE )
        {
            SetConVarInt(slaylosers_enabled, 1);
        }
    }
    ...
    if (g_enabled && isFighting)
    {
        ...
        if (loser == ctid || loser == tid)
        {
            ...
            if ( (winner != loser) && (winner != 0) )
            {
                ...
                {
                    CPrintToChatEx(winner, winner, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", winnername, "has won");
                }
    ...
    if ( clientid == g_winnerid )
    {
        if ( g_winnerhealth > 0 || g_winnermoney > 0 || g_winnerspeed > 0 )
        {
            ...
            if (g_winnermoney > 0)
            {
                ...
                Format(buffer, sizeof(buffer), "\n%t: $%i", "Money", g_winnermoney);
                StrCat(itemswon, sizeof(itemswon), buffer);
            }
            if (g_winnerspeed > 0)
            {
                SetEntData(clientid, g_iSpeed, g_winnerspeed);
                Format(buffer, sizeof(buffer), "\n%t: %.2fx", "Speed", g_winnerspeed);
                StrCat(itemswon, sizeof(itemswon), buffer);
            }
...
SendKnifeMenus(ct,t)
{
    ...
    {
        CPrintToChatEx(ct, ct, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", ctname, "Player agrees");
        ctagree = 1;
    }
    else if (g_fightPrefs[ct] == -1)
    {
        CPrintToChatEx(ct, ct, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", ctname, "Player disagrees");
        ctagree = 0;
    ...
    {
        CPrintToChatEx(t, t, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", tname, "Player agrees");
        tagree = 1;
    }
    else if (g_fightPrefs[t] == -1)
    {
        CPrintToChatEx(t, t, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", tname, "Player disagrees");
        tagree = 0;
        EmitSoundToAll(g_declinesound);
    }
    else
    ...
    {
        CPrintToChatAll("{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {GREEN}%t", "Both agree");
        CreateTimer(0.5, DelayFight);
    }
}
...
public PanelHandler(Handle:menu, MenuAction:action, client, item)
{
    if (action == MenuAction_Select)
    {
        if (item == 1 && client != 0)
        {
            ...
            {
                CPrintToChatEx(client, client, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", ctname, "Player agrees");
                ctagree = 1;
            } 
            else if (GetClientTeam(client) == 2 && client == tid && IsClientInGame(ctid) && IsPlayerAlive(ctid))
            {
                CPrintToChatEx(client, client, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", tname, "Player agrees");
                tagree = 1;
            }
        }
        else if (item == 2 && client != 0)
        {
            if (GetClientTeam(client) == 3 && client == ctid && IsClientInGame(tid) && IsPlayerAlive(tid))
            {
                CPrintToChatEx(client, client, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", ctname, "Player disagrees");
                ctagree = 0;
                EmitSoundToAll(g_declinesound);
            } 
            else if (GetClientTeam(client) == 2 && client == tid && IsClientInGame(ctid) && IsPlayerAlive(ctid))
            {
                CPrintToChatEx(client, client, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {TEAMCOLOR}%s {GREEN}%t", tname, "Player disagrees");
                tagree = 0;
                EmitSoundToAll(g_declinesound);
            }
        }
    ...
    else if ( action == MenuAction_Cancel )
    {
        ...
        {
            CPrintToChat(client, "{chartreuse}[{dodgerblue}KnifeFight{chartreuse}] {GREEN}%t", "Restore fight menu");
        }
    }
