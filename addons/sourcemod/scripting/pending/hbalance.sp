...
#define PLUGIN_VERSION "1.6.6-private"
...
public OnPluginStart()
{
	...
	CreateConVar("hbalance_version", PLUGIN_VERSION, "Version of [HANSE] Team Balance", FCVAR_NOTIFY|FCVAR_SPONLY);
	...
	s_minPlayers = CreateConVar("sm_balance_min_players", "5", "Minimum players in game to start balancing",0, true, 2.0);
...
public Action Timer_Balance(Handle timer, any winner)
{
	...
	if (balanced)
	{
		if (GetConVarInt(s_logLevel) != LOG_NEVER) {
			CPrintToChatAll("{chartreuse}[{dodgerblue}Balance{chartreuse}]{default} %T", "Teams have been balanced", LANG_SERVER);
		}
		#if defined DEBUG
	...
	else 
	{
		if (GetConVarInt(s_logLevel) == LOG_ALWAYS) {
			CPrintToChatAll("{chartreuse}[{dodgerblue}Balance{chartreuse}]{default} %T", "No team balancing necessary", LANG_SERVER);
		}
	}
}
		...
		for (new i=0; i < getTeamTableSize(getOpposingTeamOf(targetTeam)); i++)
		{
			...
			if (id == client)
			{
				...
				if(targetTeam == CS_TEAM_T) {
					...
					PrintCenterText(client, "[Balance] %T", "You have been switched to the T side.", client);
				}
				if(targetTeam == CS_TEAM_CT) {
					Client_ScreenFade(client, 1000, FFADE_OUT|FFADE_PURGE, 100, 0, 0, 255, 60);
					EmitSoundToClient(client, "common/warning.wav");
					PrintCenterText(client, "[Balance] %T", "You have been switched to the CT side.", client);
				}
