...
#pragma semicolon 1
#define PLUGIN_VERSION "3.5fix-morecolors"
...
public Action msg(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}RoundEndSound{chartreuse}]{DEFAULT} %t", "JoinMsg");
	}
}
...
void RefreshSounds(int client)
{
	...
	if(SamePath)
	{
		ReplyToCommand(client, "[RoundEndSound] SOUNDS: %d sounds loaded from \"sound/%s\"", LoadSounds(trSoundsArray, g_hTRPath), trSoundPath);
	}
	else
	{
		ReplyToCommand(client, "[RoundEndSound] CT SOUNDS: %d sounds loaded from \"sound/%s\"", LoadSounds(ctSoundsArray, g_hCTPath), ctSoundPath);
		ReplyToCommand(client, "[RoundEndSound] TR SOUNDS: %d sounds loaded from \"sound/%s\"", LoadSounds(trSoundsArray, g_hTRPath), trSoundPath);
	}
	...
	if(RoundDrawOption != 0)
		switch(RoundDrawOption)
		{
			case 1:
			{
				...
				ReplyToCommand(client, "[RoundEndSound] DRAW SOUNDS: %d sounds loaded from \"sound/%s\"", trSoundsArray.Length, trSoundPath);
			}
			case 2:
			{
				drawSoundsArray = ctSoundsArray;
				g_hDrawPath = g_hCTPath;
				ReplyToCommand(client, "[RoundEndSound] DRAW SOUNDS: %d sounds loaded from \"sound/%s\"", ctSoundsArray.Length, ctSoundPath);
			}
			default:
			{
				...
				if(!StrEqual(drawSoundsPath, ""))
					ReplyToCommand(client, "[RoundEndSound] DRAW SOUNDS: %d sounds loaded from \"sound/%s\"", LoadSounds(drawSoundsArray, g_hDrawPath), drawSoundsPath);
			}
		}
...
void PlayMusicAll(char[] szSound)
{
	...
	if(GetConVarInt(g_PlayPrint) == 1)
	{
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}RoundEndSound{chartreuse}]{DEFAULT} %t", "mp3 print", !StrEqual(soundPrint, "") ? soundPrint : szSound);
	}
}
