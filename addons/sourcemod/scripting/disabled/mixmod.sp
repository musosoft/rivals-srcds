...
#define MODNAME "MIX"
// Plugin version:
#define PLUGIN_VERSION "4.4-private"
...
public OnPluginStart()
{
	...
	g_CvarCustomLiveCfg = CreateConVar("sm_mixmod_custom_live_cfg", "match.cfg", "Custom name of the mr15 (live or match) config: (If the name doesn't exist, the plugin will try to execute match/live/mr15/esl5on5 cfg)");
	g_CvarCustomPracCfg = CreateConVar("sm_mixmod_custom_prac_cfg", "warmp.cfg", "Custom name of the prac (warmup) config: (If the name doesn't exist, the plugin will try to execute prac / warmup config)");
	g_CvarCustomMr3Cfg = CreateConVar("sm_mixmod_custom_mr3_cfg", "overtime.cfg", "Custome name of the mr3 config: (If the name doesn't exist, the plugin will try to execute mr3.cfg)");
	...
	g_CvarEnableKnifeRound = CreateConVar("sm_mixmod_enable_knife_round", "1", "Before the mix starts, do knife round? 0 - Disable, 1 - Enable.");
	...
	g_CvarDelayBeforeSwapping = CreateConVar("sm_mixmod_time_before_swapping_teams", "0.4", "In seconds: how long should the plugin wait before swapping teams when half ends?");
	g_CvarShowTkMessage = CreateConVar("sm_mixmod_show_tk_damage", "0", "Inform all the players in the server when TK (Team killing / team damage) has been done? 0 - No, 1 - Yes.");
	...
	g_CvarShowMVP = CreateConVar("sm_mixmod_show_mvp", "0", "Show the mvp? 0 - No, 1 - Yes.");
	...
	// This plugin version
	g_hPluginVersion = CreateConVar("sm_mixmod_version", PLUGIN_VERSION, "Mix Plugin version.", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	if (g_iAccount == -1)
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Can't find the m_iAccount offest! - Money wont be shown in the mix!", MODNAME);
	...
	{
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't find the cvar mp_restartgame !", MODNAME);
	...
	{
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't find the cvar mp_freezetime !", MODNAME);
	...
	if (g_iToolsAmmo == -1)
	{
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't find the offest: m_iAmmo!", MODNAME);
...
public Action:InformPlayerAboutTheMix(Handle:timer, any:client)
{
	if (hasMixStarted)
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is running! \x01Good Luck \x03And\x01 Have Fun!", MODNAME);
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is running! \x01Good Luck \x03And\x01 Have Fun!", MODNAME);
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is running! \x01Good Luck \x03And\x01 Have Fun!", MODNAME);
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is running! \x01LIVE-LIVE-\x04LIVE\x01-LIVE-LIVE", MODNAME);
	}
}
...
public Action:Command_GagPlayer(client, args) {
	...
	if (count <= 0)
	{
		CPrintToChat(client,"\x04[%s] |\x03No such a target %d", MODNAME, pattern);
		return Plugin_Handled;
	}
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have gagged %s !", MODNAME, targetName);
			CPrintToChat(targets[i], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been gagged!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_UnGagPlayer(client, args) {
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have un-gagged %s !", MODNAME, targetName);
			CPrintToChat(targets[i], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been un-gagged!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_MutePlayer(client, args)
{
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have muted %s !", MODNAME, targetName);
			CPrintToChat(targets[i], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been muted!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_UnMutePlayer(client, args)
{
	...
	else
	{
		for (new i = 0; i < count; i++)
		{
			...
			GetClientName(targets[i], targetName, sizeof(targetName));
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have un-muted %s !", MODNAME, targetName);
			CPrintToChat(targets[i], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been un-muted!", MODNAME);
		}
	}
	return Plugin_Handled;
}
...
public Action:Command_Last(client, args)
{
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} No player has joined since the plugin loaded...", MODNAME);
		return Plugin_Handled;
	}
	
	CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The last player that joined is:", MODNAME);
	CPrintToChatAll("\x04[\x03%s\x04] \x03%s .", g_LastEntered_SteamID, g_LastEntered_Name);
...
public Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	*/
		...
		if (IsClientInGame(i))
		{
			...
			CPrintToChat(i, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Your weapons, money and score has been refreshed!", MODNAME);
		}
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (isKo3Running) // Knife is running and its own text will be shown.
		{
			if (GetConVarInt(g_CvarHalfAutoLiveStart) == 1)
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Ko3 is running...", MODNAME);
			else if (hasMixStarted)
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Ko3 is running... type !live to start the match", MODNAME);
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} This round is decisive on who will pick their side.", MODNAME);
			return;
		}
		...
		if (hasMixStarted)
		{
			...
			if (g_CurrentHalf == 1)
			{
				...
				if (GetConVarInt(g_CvarShowScores) == 1)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScoreH1, teamBName, g_nTScoreH1);
				
				if (!didLiveStarted)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Not Live!", MODNAME);
			}
			else if (g_CurrentHalf == 2)
			{
				...
				else
				{
					...
					if (GetConVarInt(g_CvarShowScores) == 1)
						CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
					...
					if ((g_nCTScore == 15) && (g_nTScore == 15) && (GetConVarInt(g_CvarMr3Enabled) == 1)) // If roundEnd failed at starting the mr3 settings...
					{
						...
						if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
							CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. Mr3 settings are loaded! \x01- \x03Type\x04 !live \x03to start.", MODNAME);
						else
							CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. Mr3 settings are loaded!", MODNAME);
					}
					else
					{
						if (GetConVarInt(g_CvarMr3Enabled) == 1)
						{
							if ((g_nCTScore == 15) && (g_nTScore == 14))
								CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} LR \x04for\x03 %s - \x04If %s wins, Mr3 will be loaded!", MODNAME, teamAName, teamBName);
							else if ((g_nCTScore == 14) && (g_nTScore == 15))
								CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} LR \x04for\x03 %s - \x04If %s wins, Mr3 will be loaded!", MODNAME, teamBName, teamAName);
							else
							{
								if (g_nCTScore == 15)
									CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} LR \x04for\x03 %s", MODNAME, teamAName);
								if (g_nTScore == 15)
									CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} LR \x04for\x03 %s", MODNAME, teamBName);
							}
						}
						else
						{
			...
			else if (g_CurrentHalf > 2)
			{
				...
				if (GetConVarInt(g_CvarShowScores) == 1)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 4\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
			...
			{   
				...
				if ((g_CurrentHalf == 1) && (g_CurrentRound == 16))
				{
					...
					if (GetConVarInt(g_CvarShowSwitchInPanel) == 1) // 0 = in chat, 1 = in panel.
					{
						...
						CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams will be swaped AUTOMATICALLY after this round. Do \x01NOT\x03 change your team.", MODNAME);
				}
				else if ((g_CurrentHalf == 3) && (g_CurrentRound == 4))
				{
					...
					if (GetConVarInt(g_CvarShowSwitchInPanel) == 1) // 0 = in chat, 1 = in panel.
					{
						...
						CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams will be swapped AUTOMATICALLY after this round. Do \x01NOT\x03 change your team.", MODNAME);
				}
			}
...
public Handle_TeamsVoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_VoteEnd)
	{
		...
		isKo3Running = false;
		CPrintToChatAll("x04[%s]:\x03 Teams has been decided!");
		Command_Mr15(0, 0);
	}
}
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (isKo3Running) // Knife round has been ended...
		{
			if ((GetConVarInt(g_CvarKnifeWinTeamVote) == 1)) // Lets vote for the side of the teams.
			{
				...
				if (win_team == 2)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Terrorists will choose now their team!", MODNAME);
				else if (win_team == 3)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Counter-Terrorists will choose now their team!", MODNAME);
		...
		if ((hasMixStarted) && (didLiveStarted)) // Add 1 to the current round win number
		{
			...
			if (GetConVarInt(g_CvarShowMVP) == 1)
			{
				...
				if (kills == 5)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x01 %d \x03MVP:\x04 %s , \x03Did an \x04ACE!", MODNAME, g_CurrentRound-1, attackerName, kills);
				else if (kills == 4)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x01 %d \x03MVP:\x04 %s , \x03Did a \x04MINI!", MODNAME, g_CurrentRound-1, attackerName, kills);
				else if (kills == 3)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x01 %d \x03MVP:\x04 %s , \x03Killed:\x04 %d \x03Enemies!", MODNAME, g_CurrentRound-1, attackerName, kills);
			}
		}
		...
		if ((hasMixStarted) && (g_SwapNow) && (didLiveStarted) && (g_CurrentHalf == 1))
		{
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Swapping teams...", MODNAME);
			...
			if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. HALF 2! \x01- \x03Type\x04 !live \x03to start.", MODNAME);
			else
			{
				new executed = 0;
				...
				if (executed == 0) // Couldn't execute the zb_lo3 command
				{
					if(GetConVarInt(g_CvarRestartTimeInLiveCommand) > 0)
						...
						CreateTimer(1.0, ThreeRestarts);
				}
				
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. HALF 2 Will begin in a few seconds!", MODNAME);
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. HALF 2 Will begin in a few seconds!", MODNAME);
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. HALF 2 Will begin in a few seconds!", MODNAME);
			}
		...
		else if ((hasMixStarted) && (didLiveStarted))
		{
			...
			if ((g_CurrentHalf == 3) && (g_SwapNow) && (g_nCTScore < 19) && (g_nTScore < 19))
			{
				...
				if (GetConVarInt(g_CvarHalfAutoLiveStart) == 0)
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. LAST HALF! \x01- \x03Type\x04 !live \x03to start.", MODNAME);
				else
					CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams swapped. LAST HALF!", MODNAME);
				g_SwapNow = false;
			}
...
public Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarShowTkMessage) == 1) && (hasMixStarted) && (!g_SaveClientsScore))
	{
		...
		if ((attacker > 0) && (attacker <= MaxClients) && (GetClientTeam(victim) == GetClientTeam(attacker)) && (victim != attacker)) // Check to see if it is not by an explosive / fall damage, and he is a teammate.
		{
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s teamattacked %s with\x01 %d \x03HP,\x01 %d\x03 armor!", MODNAME, attackerName, victimName, GetEventInt(event, "dmg_health"), GetEventInt(event, "dmg_armor"));
		}
	}
}
...
public Action:Command_TvRecord(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Source-Tv is already recording. You can either stop the record, or wait for the end of the mix.", MODNAME);
			return Plugin_Handled;
		}
...
public Action:Command_Pcw(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (hasMixStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is already running.", MODNAME);
			return Plugin_Handled;
		}
		
		if (GetConVarInt(g_CvarShowMVP) != 1)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} To use sm_pcw, MVP Scores must be shown!", MODNAME);
			return Plugin_Handled;
		}
		...
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} This plugin is disabled ... To enable it: \x01sm_mixmod_enable 1", MODNAME);
...
public Action:Command_Stop(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			isKo3Running = false;
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knife round has been disabled!", MODNAME);
			return Plugin_Handled;
		}

		if (!hasMixStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is not running...", MODNAME);
			return Plugin_Handled;
		}
...
public Action:Command_Live(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (didLiveStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Live is already running!", MODNAME);
			return Plugin_Handled;
		}
		if (!hasMixStarted)
		{
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Live is running!", MODNAME);
...
public Action:Command_NotLive(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!didLiveStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Live is not running...", MODNAME);
			return Plugin_Handled;
		}
		if (!hasMixStarted)
		{
...
public Action:Command_RandomTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (hasMixStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is already running. You can't random teams now!", MODNAME);
			return Plugin_Handled;
		}
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams have been randomized!", MODNAME);
...
RandomCTPlayers()
{
	...
	while(numSwitched < 5)
	{
		...
		else
		{
			...
			CPrintToChatAll("\x04[%s]:\x01 ERROR: Failed to get a random player from T team!", MODNAME);
			break;
		}
	}
}
...
public Action:Command_Pause(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		g_CurrentRound--;
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} This round won't be counted!", MODNAME);
...
public Action:InformMix(Handle:timer, any:team)
{
	...
	CPrintToChatAll("\x04--- Mix-Plugin created by iDragon ---");
	CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The mix has Ended!", MODNAME);
	if (team == 3)
		CPrintToChatAll("\x04[%s]: \x03The winner is:\x04 %s", MODNAME, teamAName);
	else if (team == 2)
		CPrintToChatAll("\x04[%s]: \x03The winner is:\x04 %s", MODNAME, teamBName);
	else if (team == 1)
		CPrintToChatAll("\x04[%s]: \x03There was no winner.\x04 IT'S A DRAW!", MODNAME);
...
public Action:DisplayScores(Handle:timer)
{
	...
	if (win < 1)
	{
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} There was an error trying to find the mvp player...", MODNAME);
		return;
	}
	decl String:winnerName[33];
	GetClientName(win, winnerName, sizeof(winnerName));
	//new kills = g_ScoresOfTheGame[win];
	//CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Kills Stat:", MODNAME);
	//CPrintToChatAll("------------------");
	//CPrintToChatAll("\x03 - The \x01MVP:\x04 %s , \x03Killed:\x04 %d \x03Enemies!", winnerName, kills);
...
public Action:Command_Mr15(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (FileExists(cfgPath))
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't execute the %s / match / mr15 / live / esl5on5 config!", MODNAME, cfgName);
...
public Action:Command_Prac(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (FileExists(cfgPath))
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't execute the %s / warmup / prac config!", MODNAME, cfgName);
...
public Action:Command_Mr3(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (GetConVarInt(g_CvarMr3Enabled) == 1)
		{
			...
			{
				ServerCommand("exec %s", cfgName);
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s Config has been loaded...", MODNAME, cfgName);
			}
			else if (FileExists("cfg/mr3.cfg"))
			{
				ServerCommand("exec mr3");
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} MR3 Config has been loaded...", MODNAME);
			}
			else
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Couldn't execute the mr3 config!", MODNAME);
		}
		else CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mr3 is disabled... mr3 settings will not be loaded!", MODNAME);
...
public Action:Command_KO3(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Live is running... ko3 settings won't load!", MODNAME);
			return Plugin_Handled;
		}
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knives round!", MODNAME);
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knives round!", MODNAME);
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knives round!", MODNAME);
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knives round!", MODNAME);
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Knives round!", MODNAME);
	...
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		if (args < 1)
		{
			...
			if (GetUserAdmin(client) != INVALID_ADMIN_ID)
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The server password is: %s", MODNAME, sPass);
			else
			{
				if ((isRandomPasswordWasLastPw) && (GetConVarInt(g_CvarRpwShowPass) == 0))
				{
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The server password is: ***RANDOM***", MODNAME);
					if (GetUserAdmin(client) != INVALID_ADMIN_ID)
						CPrintToChat(client, "\x04*** %s", sPass);
				}
				else
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The server password is: %s", MODNAME, sPass);
			}
		}
		else
		{
			if (GetUserAdmin(client) != INVALID_ADMIN_ID)
			{
				...
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Server password is now: %s", MODNAME, pass);
				isRandomPasswordWasLastPw = false;
			}
			else
				CPrintToChat(client, "[SM] Only admins can change the password!");
		}
...
public Action:Command_GenerateRandomPassword(client, args)
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		...
		if ((GetConVarInt(g_CvarRpwShowPass) == 1) || (!g_IsItManual && (client == 0))) // Show the password
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The server password is: %d", MODNAME, rnd);
		else if (GetConVarInt(g_CvarRpwShowPass) == 0) // Don't show the password
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The server password is: ***RANDOM***", MODNAME);
			// Show the real password to the admin.
			if (client != 0)
				CPrintToChat(client, "\x04*** %d", rnd);
			else
...
public Action:Command_RemovePass(client, args)
{
	if ((GetConVarInt(g_CvarEnabled) == 1) && (GetConVarInt(g_CvarEnablePasswords) == 1))
	{
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Password has been removed!", MODNAME);
		isRandomPasswordWasLastPw = false;
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (args == 0)
		{
			...
			for(new i=1;i<=MaxClients;i++)
			{
				...
				{
					ChangeClientTeam(i, 3);
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been swapped to \x01 Counter Terrorists\x03 Team.", MODNAME);
				}
				else if (team == 3)
				{
					ChangeClientTeam(i, 2);
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been swapped to \x01 Terrorists\x03 Team.", MODNAME);
				}
			}
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} All players have been swapped.", MODNAME);
		...
		else if (args >= 2)
		{
			...
			else
			{
				...
				if ((team <= 0) || (team > 3))
				{
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Usage: sm_st <player> [team] (legal teams: t,ct,spec,1,2,3)", MODNAME);
					return Plugin_Handled;
				}
			}
			...
			if (found > 0) // Target is found!
			{
				...
				for (new target = 0; target < found; target++)
				{
					if (GetClientTeam(targetsArr[target]) != team)
					{
						...
						CPrintToChat(targetsArr[target], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Your team has been changed!", MODNAME);
					}
				}
				if (flag)
					didLiveStarted = true;
					
				CPrintToChat(client,"{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s team has been changed!", MODNAME, searchFor);
			}
			else 
				CPrintToChat(client,"\x04[%s]:\x03Couldn't find %s in the server...", MODNAME, searchFor);
...
public Action:ThreeRestarts(Handle timer) 
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (GetConVarInt(g_CvarEnableRRCommand) == 1)
...
*/
	...
	//  CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} The game has been restarted!", MODNAME);
	}
}
...
public Action:ShowScores(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!IsFakeClient(client))
		{
			if (!hasMixStarted)
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is not running yet...", MODNAME);
				return Plugin_Handled;
			}
			...
			if (g_CurrentHalf == 1)
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScoreH1, teamBName, g_nTScoreH1);
				
				if (!didLiveStarted)
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Not Live!", MODNAME);
			}
			else if (g_CurrentHalf == 2)
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 2\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
			...
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Round\x03 %d \x04- Half\x03 %d\x04 /\x03 4\x04 - %s\x03 %d,\x04 %s\x03 %d\x04.", MODNAME, g_CurrentRound, g_CurrentHalf, teamAName, g_nCTScore, teamBName, g_nTScore);
...
public Action:Command_SetMixScore(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1 && (ENABLE_PLUGIN_CHECKING))
	{
		if(args < 3)
		{
			CPrintToChat(client, "[Usage]: sm_setmixscore <half> <ct> <t>");
			return Plugin_Handled;
		}
		...
		CPrintToChat(client, "Scores has been set!");
...
public Action:Command_KickCT(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Counter-Terrorist team has been kicked!", MODNAME);
...
public Action:Command_KickT(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Terrorist team has been kicked!", MODNAME);
...
public Action:Command_Maps(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (isMapListGenerated) // Is map list already been generated?
		{
			...
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Choose a map to changelevel!", MODNAME);
		}
		else // The maplist hasn't been generated yet ... Lets create it now and infrom the admin.
		{
			CreateMapList();
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Map list is being created right now! please use\x01 sm_maps\x03 command agaon in a few seconds...", MODNAME);
		}
...
CreateMapList()
{
	if (!isMapListGenerated) // Map list will be generated now!
	{
		...
		switch(GetConVarInt(g_CvarMapListFrom)) // Read from: 0 = Maps dir, 1 = mapcycle.txt
		{
			...
			case 1:
			{
				new Handle:mapsFile = OpenFile("cfg/mapcycle.txt","r");
...
public MapListMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Changing map to:\x01 %s", MODNAME, map);
			ServerCommand("changelevel %s", map);
		}
		else
			CPrintToChat(param1, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Failed to change map to:\x01 %s", MODNAME, map); // Inform the admin that the map couldn't be changed.
	}
}
...
public Action:Command_DisableChat(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			case 0:
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Public chat is enabled!", MODNAME);
			case 1:
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Public chat is disabled!", MODNAME);
			case 2:
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Public chat is disabled when live is running!", MODNAME);
		}
...
public Action:Command_MixHelp(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CreateHelpPanel();
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Try again in a few seconds!", MODNAME);
		}
...
public HelpMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		CPrintToChat(param1, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s", MODNAME, info);
	}
	return 0;
}
	...
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (found > 0) // Target is found!
		{
			for (new target = 0; target < found; target++)
			{
				...
				CPrintToChat(targetsArr[target], "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You have been moved to the spectors team!", MODNAME);
			}
			CPrintToChat(client,"{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s has been moved to Spectors team!", MODNAME, searchFor);
		}
		else 
			CPrintToChat(client,"\x04[%s]:\x03Couldn't find %s in the server...", MODNAME, searchFor);
...
public Action:Command_MixMenu(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CreateMixMenu();
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix Menu is being created right now! please use\x01 sm_mix\x03 command again in a few seconds...", MODNAME);
		}
		...
		*/
		switch (param2) 
		{
			...
			case 4:
			{
				...
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} All players are in spectors team!", MODNAME);
			}
			case 5:
...
public SwapPlayerMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		if (param2 > 0) // Player was selcted ...
		{
			...
			CPrintToChat(param1, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s has been swapped!", MODNAME, info);
			CPrintToChat(target, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You has been swapped!", MODNAME);
		}
		...
		switch (param2)
		{
			...
			*/
			...
			case 3:
			{
				...
				CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Spectors team has been kicked!", MODNAME);
			}
			case 4:
			{
				decl String:clientName[128], String:useridS[20];
...
public KickPlayerMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		if (param2 > 0) // Player was selcted ...
		{
			...
			GetClientName(target, info, sizeof(info));
			CPrintToChat(param1, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} %s has been kicked!", MODNAME, info);
		}
		else if (param2 == 0) // Back to admin Menu
			DisplayMenu(g_AdminMenu, param1, 30);
	}
...
public Event_PlayerDisconnect(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((GetConVarInt(g_CvarAutoMixEnabled) == 1) && !g_IsItManual) // Auto-Mix is running...
	{
		...
		if (g_ReadyPlayers[client])
		{
			...
			g_AllowReady = true;
			Command_RemovePass(0, 0);
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Sub is needed! password has been removed!");
			*/
		}
...
public Action:Command_JoinTeam(client, args)
{
	...
	{
		CPrintToChat(client,"{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You can't change your team when you are ready!", MODNAME);
		return Plugin_Handled;
	}
...
public Action:Command_JoinTeam(client, args)
{
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You can not change your team when mix is running!", MODNAME);
		return Plugin_Handled;
	}
	return Plugin_Continue;
}
...
public Action:Command_Ready(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Auto-War is not running! This command is disabled.", MODNAME);
			return Plugin_Handled;
		}
		
		if (!g_AllowReady)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} There are already 10 players ready!", MODNAME);
			return Plugin_Handled;
		}
		...
		if (hasMixStarted)
		{
			...
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix is running! This command is now disabled.", MODNAME);
				return Plugin_Handled;
			}
		}
		...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You must be in Terrorist or Counter-Terrorist team to use this command.", MODNAME);
			return Plugin_Handled;
		}
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You are ready!", MODNAME);
...
public Action:Command_ShowTeams(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		...
		if (GetConVarInt(g_CvarAutoMixEnabled) == 0) // Auto-Mix is not running...
		{
			...
			{
				if (client != 0)
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams weren't randomized!", MODNAME);
				return Plugin_Handled;
			}
		}
		...
		if (g_IsItManual || !hasMixStarted)
		{
			...
			{
				if (client != 0)
					CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} This command is enabled only when mix is running");
		...
		if (client == 0)
			CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams:", MODNAME);
		else
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Teams:", MODNAME);
		...
		if (client == 0)
		{
			CPrintToChatAll(teamAPlayers);
			CPrintToChatAll(teamBPlayers);
		}
		else
		{
			CPrintToChat(client, teamAPlayers);
			CPrintToChat(client, teamBPlayers);
		}
...
public Action:Command_UnReady(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (GetConVarInt(g_CvarAutoMixEnabled) == 0) // Auto-Mix is not running...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Auto-War is not running! This command is now disabled.", MODNAME);
			return Plugin_Handled;
		}
		...
		CPrintToChatAll("{chartreuse}[{dodgerblue}%s{chartreuse}]{green} You are not ready!", MODNAME);
...
public Action:Command_ShowMvp(client, args)
{
	if (GetConVarInt(g_CvarEnabled) == 1)
	{
		if (!hasMixStarted)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} Mix/Pcw is not running! This command is now disabled.", MODNAME);
			return Plugin_Handled;
		}
		
		if (GetConVarInt(g_CvarShowMVP) == 0)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} MVP Stats is disabled! thsi command is now disabled.", MODNAME);
			return Plugin_Handled;
		}
		...
		if (kills <= 0)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} MVP wasn't chosen yet. Please wait a few minutes.", MODNAME);
			return Plugin_Handled;
		}
		//CPrintToChat(client, "{chartreuse}[{dodgerblue}%s{chartreuse}]{green} MVP:", MODNAME);
		//CPrintToChat(client, "------------------");
		//CPrintToChat(client, "\x03 - The \x01MVP:\x04 %s , \x03Killed:\x04 %d \x03Enemies!", mvpName, kills);
		...
		CPrintToChat(client, "\x03 You killed:\x04 %d \x03 Enemies.", g_ScoresOfTheGame[client]);
