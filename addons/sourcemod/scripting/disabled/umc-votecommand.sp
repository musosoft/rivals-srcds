...
public Action:Command_Vote(client, args)
{
    if (!can_vote)
    {
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Mapcycle is invalid, cannot start a vote.");
        return Plugin_Handled;
    }
    
    if (args < 1)
    {
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Usage: sm_umc_mapvote <0|1|2>\n 0: Change now, 1: Change at end of round, 2: Change at end of map.");
        return Plugin_Handled;
    }
    ...
    if (result)
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Started Vote.");
    else
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Could not start vote. See log for details.");
