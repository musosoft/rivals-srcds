...
#define PLUGIN_VERSION "1.6.0-private"
...
public OnPluginStart()
{
	...
	g_hEnabled				= CreateConVar("kassist", "1.0", "If kill assist is enabled. 1=Yes, Def. 1", FCVAR_NONE, true, 0.0, true, 1.0);
	g_hMinDmg				= CreateConVar("kassist_minDmg", "35.0", "Minimum damage required to assist a kill. Def. 35", FCVAR_NONE, true, 1.0);
	g_hAddKillerScore		= CreateConVar("kassist_killValue", "0.0", "Bonus points/frags given to a killer. Integer. Def. 0 (disabled)", FCVAR_NONE, true, 0.0);
	g_hAddAssisterScore		= CreateConVar("kassist_assistValue", "0.0", "Bonus points/frags given to an assister. Real. Def. 0.0 (disabled)", FCVAR_NONE, true, 0.0);
	g_hAddRankMeScore		= CreateConVar("kassist_rankMeValue", "0.0", 
		"RankMe points given to an assister. Integer. Def. 0 (disabled). Only use other values if you have RankMe.", FCVAR_NONE, true, 0.0);
	g_hVerbose				= CreateConVar("kassist_verbose", "2.0", "The default value of the verbose cookie. Tell players when they assist someone. 1=Yes (always), 2=Yes (when dead, def.).", FCVAR_NONE, true, 0.0, true, 2.0);
	g_hVerboseCookie		= CreateConVar("kassist_verbose_cookie", "1.0", "If verbose cookies are used. 0=No, 1=Yes (Def.), 2=Yes (remove the 'always' option).", FCVAR_NONE, true, 0.0, true, 2.0);
	...
	{
		g_hSplit			= CreateConVar("kassist_split", "1.0", "If kill assist cash is split amongst assisters. 1=Yes, Def. 1.", FCVAR_NONE, true, 0.0, true, 1.0);
	...
	{
		g_hReward			= CreateConVar("kassist_cash", "150.0", "Kill assist cash awarded to assisters. Def. 150.", FCVAR_NONE, true, 0.0);
		g_hUseMVPStars		= CreateConVar("kassist_useMVPStars", "2.0", 
		"Override MVPs (Stars) system to show assists ? 1=Yes (SDKHooks), 2=Yes (CS_SetMVPCount if available, else SDKHooks), Def. 2.", FCVAR_NONE, true, 0.0, true, 2.0);
	}
...
Action:GiveAssistPoints(assisters[], any:nbAssisters, any:killerId, victimId, csgoAssisterId/*to prevent adding assists*/, victimTeam )
{
	...
	for (new i; i < nbAssisters; ++i)
	{
		...
		{
			CPrintToChat( assisterId, "{chartreuse}[{dodgerblue}Assist{chartreuse}]{default} %t", "Assist", "{teamcolor}", killerId, "{default}", "{olive}", victimId, "{default}" );
			if ( g_bHasPrintedOnceThisMap[ assisterId ] == false && verboseCookie != 0 && verboseOncePerMap == true )
			{
				CPrintToChat( assisterId, "{chartreuse}[{dodgerblue}Assist{chartreuse}]{default} %t", "TellHowToChange", "{teamcolor}", "{default}", "{teamcolor}", "{default}" );
