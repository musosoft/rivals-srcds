...
public Action:Command_TestReweight(client, args)
{
    if (args < 1)
    {
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Usage: sm_umc_maprate_testreweight <map>");
    }
    else
    {
        ...
        CReplyToCommand(
            client,
            "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Map %s will be reweighted by a factor of %f",
