...
public Plugin:myinfo = {
    name = "AllTalk",
    author = "muso.sk",
    description = "Adds the possibility to alltalk while teamtalk is enabled",
    version = "0.1-private",
}
    ...
    if ((buttons & IN_USE) > 0 && (s_iButtons[client] & IN_USE) == 0)
    {
        if (IsPlayerAlive(client))
        {
            ...
            for (new i=1; i<=MaxClients; i++)
            {
                ...
                // This guy isn't in our team, so he should hear what i'm saying.
                SetListenOverride(i, client, Listen_Yes);
                //PrintCenterText(i, "TeamTalk ✓");
            }
        }
    }
