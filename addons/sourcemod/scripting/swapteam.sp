...
#define SWAPTEAM_VERSION	"1.2.6-private"
...
public Plugin:myinfo = {
	name = "SwapTeam",
	author = "Rogue - Originally by MistaGee, muso.sk",
...
{
	CreateConVar("swapteam_version", SWAPTEAM_VERSION, "SwapTeam Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
	...
	if (cstrikeExtAvail && !toSpec)
	{
		...
		if (CSS)
		{
			if (cTeam == TEAM_2)
			{
				...
				PrintCenterText(client, "[SwapTeam] Teams switched! You are now T.");
			}
			else
			{
				...
				EmitSoundToClient(client, "common/warning.wav");
				PrintCenterText(client, "[SwapTeam] Teams switched! You are now CT.");
			}
		}
