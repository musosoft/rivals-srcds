...
#define PLUGIN_VERSION "1.3-private"
...
{
	name = "LastMan",
	author = "dalto, G-Phoenix, muso.sk",
...
public OnPluginStart()
{
	...
	CreateConVar("sm_lastman_version", PLUGIN_VERSION, "Last Man Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
{
	if (g_Game == GAME_CSTRIKE || g_Game == GAME_OTHER)
	{
...
public Action:TimerAnnounce(Handle:timer, any:client)
{
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}LastMan{chartreuse}]{default} %t", "Lastman Menu");
	}
}
...
public EventPlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(lastManId > 0)
	{
		...
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}LastMan{chartreuse}]{default} %t", "Lastman", clientname);
		}
