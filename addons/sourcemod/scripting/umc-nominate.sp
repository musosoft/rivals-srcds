...
public Action:OnPlayerChat(client, const String:command[], argc)
{
    ...
    if (StrEqual(arg, "nominate", false))
    {
        ...
        {
            CPrintToChat(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "No Nominate Nextmap");
        }
        else //Otherwise, let them nominate.
        {
            ...
            if (next != -1)
            {
                ...
                if (!KvFindGroupOfMap(map_kv, arg, groupName, sizeof(groupName)))
                {
                    ...
                    CPrintToChat(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Could not find map \"%s\"", arg);
                }
                else
                {
                    KvRewind(map_kv);
                    ...
                    else
                    {
                        ...
                        CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Player Nomination", clientName, arg);
...
public Action:Command_Nominate(client, args)
{
    ...
    if (vote_completed || !can_nominate)
    {
        CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "No Nominate Nextmap");
    }
    else //Otherwise, let them nominate.
    {
        if (args > 0)
        {
            ...
            {
                //TODO: Change to translation phrase
                CReplyToCommand(client, "{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} Could not find map \"%s\"", arg);
            }
            else
            {
                KvRewind(map_kv);
...
public Handle_NominationMenu(Handle:menu, MenuAction:action, client, param2)
{
    switch (action)
    {
        case MenuAction_Select: //The client has picked something.
        {
            ...
            CPrintToChatAll("{chartreuse}[{dodgerblue}MapChooser{chartreuse}]{default} %t", "Player Nomination", clientName, map);
