...
public Action:OnWeaponCanUse(client, weapon)
{
	...
	if(!g_bSpamProtectPrint[client])
	{
		...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "SpecialNotAllowed", client);
		}
		else if(team == CS_TEAM_CT)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "IsRestrictedPickupCT", client, Restrict_GetRestrictValue(team, id));
		}
		else
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "IsRestrictedPickupT", client, Restrict_GetRestrictValue(team, id));
		}
...
public Action:CS_OnBuyCommand(client, const String:weapon[])
{
	...
	if(Restrict_IsWarmupRound())
	{
		if(!g_bSpamProtectPrint[client])
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "CannotBuyWarmup", client);
			g_bSpamProtectPrint[client] = true;
			CreateTimer(PRINTDELAY, ResetPrintDelay, client);
		}
	...
	if(result == CanBuy_Block || result == CanBuy_BlockDontDisplay)
	{
		if(team == CS_TEAM_CT && result != CanBuy_BlockDontDisplay)
		{
			if(Restrict_IsSpecialRound() && !Restrict_AllowedForSpecialRound(id))
				...
				CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "IsRestrictedBuyCT", client, Restrict_GetRestrictValue(team, id));
		}
		else if(team == CS_TEAM_T && result != CanBuy_BlockDontDisplay)
		{	if(Restrict_IsSpecialRound() && !Restrict_AllowedForSpecialRound(id))
				CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "SpecialNotAllowed", client);
			else
				CPrintToChat(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T %T", weaponNames[_:id], client, "IsRestrictedBuyT", client, Restrict_GetRestrictValue(team, id));
		}
