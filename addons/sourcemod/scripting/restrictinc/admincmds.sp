...
public Action:RemoveRestricted(client, args)
{
	...
	CShowActivity2(client, ADMINCOMMANDTAG, "%t", "RemovedRestricted");
...
public Action:RestrictAdminCmd(client, args)
{
	if(args < 1)
	{
		CReplyToCommand(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "RestrictReply", client);
		return Plugin_Handled;
	}
	...
	if(args >= 2)
	{
		...
		{
			CReplyToCommand(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "InvalidAmount", client);
			return Plugin_Handled;
		}
	}
	new teams = 0;
	if(args == 3)
	{
		...
		{
			CReplyToCommand(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "InvalidTeam", client);
			return Plugin_Handled;
		}
	}
...
public Action:UnrestrictAdminCmd(client, args)
{
	if(args < 1)
	{
		CReplyToCommand(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "UnrestrictReply", client);
		return Plugin_Handled;
	}
	decl String:weapon[100];
	GetCmdArg(1, weapon, sizeof(weapon));
...
public Action:DropC4(client, args)
{
	...
	for(new i = 1; i <= MaxClients; i++)
	{
		...
		if((bomb = GetPlayerWeaponSlot(i, _:SlotC4)) != -1)
		{
			...
			CShowActivity2(client, ADMINCOMMANDTAG, "%t", "ForcedBombDrop");
...
public Action:KnifeRound(client, args)
{
	...
	{
		CReplyToCommand(client, "{chartreuse}[{dodgerblue}Restrict{chartreuse}]{DEFAULT} %T", "SpecialRoundAlreadySet", client);
		return Plugin_Handled;
	}
	CShowActivity2(client, ADMINCOMMANDTAG, "%t", "KnivesRoundSetup");
...
public Action:PistolRound(client, args)
{
	...
	CShowActivity2(client, ADMINCOMMANDTAG, "%t", "PistolRoundSetup");
...
public Action:ReloadRestrict(client, args)
{
	...
	CShowActivity2(client, ADMINCOMMANDTAG, "%t", "ReloadedRestricitions");
