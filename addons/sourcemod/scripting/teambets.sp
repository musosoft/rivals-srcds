...
#define PLUGIN_VERSION "2.6-private"

public Plugin:myinfo = 
{
	name = "Team Bets",
	author = "GrimReaper - Original by ferret, muso.sk",
...
public Action:Command_Say(client, args)
{
 	...
 	if (strcmp(szParts[0],"bet",false) == 0)
	{
		...
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "No bets after bomb planted");
			return Plugin_Handled;
		}
	
		if (GetClientTeam(client) <= 1)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Must Be On A Team To Vote");
			return Plugin_Handled;
		}
		
		if (g_bPlayerBet[client])
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Already Betted");
			return Plugin_Handled;	
		}

		if (GetConVarInt(g_hSmBetDeadOnly) == 1 && IsAlive(client))
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Must Be Dead To Vote");
			return Plugin_Handled;
		}
	
		if (strcmp(szParts[1],"ct",false) != 0 && strcmp(szParts[1],"t", false) != 0)
		{
			CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Invalid Team for Bet");
			return Plugin_Handled;
		}
		...
		if (strcmp(szParts[1],"ct",false) == 0 || strcmp(szParts[1],"t", false) == 0)
		{
			...
			if (iAmount < 1)
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Invalid Bet Amount");
				return Plugin_Handled;
			}		
	
			if (iAmount > iBank || iBank < 1)
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Not Enough Money");
				return Plugin_Handled;
			}
			...
			{
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Players Are Dead");
				return Plugin_Continue;		
			}
			...
			if (g_iPlayerBetData[client][BET_TEAM] == 2) // 2 = t, 3 = ct
			{
				...
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Bet Made", iOdds[1], iOdds[0], iWin, g_iPlayerBetData[client][BET_AMOUNT]);		
			}
			else
			{
				iWin = RoundToNearest((float(iOdds[0]) / float(iOdds[1])) * float(iAmount));
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Bet Made", iOdds[0], iOdds[1], iWin, g_iPlayerBetData[client][BET_AMOUNT]);
			}
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		CPrintToChatAll("{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %T", "One Vs One", LANG_SERVER);
		return;
	}
	...
	if (GetConVarInt(g_hSmBetOneVsMany) > 1)
	{
		...
		if ((iTeams[0] == 1 && iTeams[1] > GetConVarInt(g_hSmBetOneVsMany)) || (iTeams[1] == 1 && iTeams[0] > GetConVarInt(g_hSmBetOneVsMany)) && !g_bOneVsMany)
		{
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %T", "One Vs Many Start", LANG_SERVER, pname, (iTeams[1] == 1 ? "Terrorist" : "Counter-Terrorist"));
		}
		else if (iTeams[0] == 1 && iTeams[1] == 0 && g_bOneVsMany && g_iOneVsManyTeam == 2)
		{
			GetClientName(iTPlayer, pname, 64);
			CPrintToChatAll("{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %T", "One Vs Many Winner", LANG_SERVER, pname, g_iOneVsManyPot);
	...
	for (new i = 1; i <= iMaxClients; i++)
	{
		if (IsClientInGame(i) && g_bPlayerBet[i])
		{
			if (iWinner == g_iPlayerBetData[i][BET_TEAM])
			{
				...
				CPrintToChat(i, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Bet Won", g_iPlayerBetData[i][BET_WIN], g_iPlayerBetData[i][BET_AMOUNT]);
			}
			else
			{
				CPrintToChat(i, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Bet Lost", g_iPlayerBetData[i][BET_AMOUNT]);
			}
		}
	...
	if (GetConVarInt(g_hSmBetAdvert) == 1)
		{
			if (IsClientInGame(client))
				CPrintToChat(client, "{chartreuse}[{dodgerblue}TeamBets{chartreuse}]{default} %t", "Advertise Bets");
