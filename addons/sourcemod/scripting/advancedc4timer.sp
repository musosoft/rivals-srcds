...
public OnPluginStart()
{
	...
	if(g_bEnable && g_bLateLoad)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i))
			{
				GetClientAuthId(i, AuthId_Steam2, g_sSteam[i], sizeof(g_sSteam[]));
...
public OnClientPostAdminCheck(client)
{
	if(g_bEnable)
	{
		GetClientAuthId(client, AuthId_Steam2, g_sSteam[client], sizeof(g_sSteam[]));
		...
		for(new i = 1; i <= MaxClients; i++)
		{
			if(IsClientInGame(i) && !IsFakeClient(i))
			{
				...
				if(!(g_iAltSound & TEXT_AT_TEN && (soundKey < 1 || soundKey > 10)))
				{
					if(g_Prefs[i][bChat])
					{
						CPrintToChat(i, "{chartreuse}[{dodgerblue}C4Timer{chartreuse}]{default} %s", sBuffer);
					}
...
public Action:Timer_Announce(Handle:timer, any:userid)
{
	...
	if(client && IsClientInGame(client))
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}C4Timer{chartreuse}]{default} %t", "announce");
	}
}
