...
#define PLUGIN_VERSION "2.2.1-morecolors"

#define CHAT_BANNER "{chartreuse}[{dodgerblue}Comms{chartreuse}]{DEFAULT} %t"
...
public OnPluginStart()
{
	...
	gH_Cvar_Announce = CreateConVar("sm_voicecomm_announce", "1", "Enable or disable the messages: 0 - disabled, 1 - enabled", 0, true, 0.0, true, 1.0);
	//gH_Cvar_VoiceComm = CreateConVar("sm_voicecomm", "0", "Controls voice communications. 0-Normal, 1-Muted after death, 2-Dead talk to dead, 3-Dead ignore teams, 4-Dead talk to living teammates.", 0, true, 0.0, true, 4.0);
	gH_Cvar_TeamTalk = CreateConVar("sm_voicecomm_teamtalk", "0", "Controls talking when alive if deadtalk is set to -2. 0 - Players talk to both teams when alive, 1 - Players talk to only their teammates when alive", 0, true, 0.0, true, 1.0);
	gH_Cvar_MuteTime = CreateConVar("sm_voicecomm_mutetime", "29.0", "Controls the length of time terrorists are muted starting from the beginning of the round.", 0, true, 2.5);
	gH_Cvar_DeadHearAlive = CreateConVar("sm_voicecomm_deadhearalive", "1", "Controls hearing after death. 0 - Dead players do not hear alive players. 1 - Dead hear alive.", 0, true, 0.0, true, 1.0);
	gH_Cvar_MuteAtStart = CreateConVar("sm_voicecomm_startmuted", "1", "Controls whether a team starts the round muted.", 0, true, 0.0, true, 1.0);
	gH_Cvar_TeamToMute = CreateConVar("sm_voicecomm_mutedteam", "2", "Defines the team to mute if startmuted is enabled. In CS:S, 2=Terrorists, 3=CTs", 0, true, 1.0, true, 3.0);
	gH_Cvar_RoundEndAllTalk = CreateConVar("sm_voicecomm_roundendalltalk", "1", "Controls whether all talk is turned on between the time between rounds", 0, true, 0.0, true, 1.0);
	gH_Cvar_MuteSpectators = CreateConVar("sm_voicecomm_spectatormute", "1", "Controls whether spectators are automatically muted. (Admins are exempt).", 0, true, 0.0, true, 1.0);
	gH_Cvar_BlockUnMuteOnRetry = CreateConVar("sm_voicecomm_blockunmuteonreconnect", "1", "Controls whether players will automatically be unmuted when rejoining or if previous mutes are enforced.", 0, true, 0.0, true, 1.0);
	gH_Cvar_AdmOvrd_FirstJoin = CreateConVar("sm_voicecomm_admovrd_firstjoin", "1", "If enabled admins who first join will be allowed to talk.", 0, true, 0.0, true, 1.0);
	gH_Cvar_AdmOvrd_OnDeath = CreateConVar("sm_voicecomm_admovrd_death", "1", "If enabled admins who die are allowed to talk with everyone.", 0, true, 0.0, true, 1.0);
	gH_Cvar_AdmOvrd_Spec = CreateConVar("sm_voicecomm_admovrd_spectate", "1", "If enabled admins speak with everyone on round start if they are a spectator.", 0, true, 0.0, true, 1.0);
...
public Action:RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarBool(gH_Cvar_MuteAtStart))
	{
		if (Announce)
		{
			CPrintToChatAll(CHAT_BANNER, "Start of Round with a Muted Team");
		}
	...
	else
	{
		if (Announce)
		{
			CPrintToChatAll(CHAT_BANNER, "Start of Round without Muted Team");
		}
	}
			...
			if (IsClientInGame(AdmIdx))
			{
				if ((GetClientTeam(AdmIdx) == SPECTATE_TEAM))
				{
					...
					if (GetAdminFlag(GetUserAdmin(AdmIdx), Admin_Chat) && GetConVarBool(gH_Cvar_AdmOvrd_Spec))
					{
						if (Announce)
						{
							CPrintToChat(AdmIdx, CHAT_BANNER, "Spectator - Admin");
						}
					...
					else
					{
						if (Announce)
						{
							CPrintToChat(AdmIdx, CHAT_BANNER, "Spectator - Not Admin");
						}
						for (new idx = 1; idx <= MaxClients; idx++)
						{
...
public Action:PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else if (g_bMuted[client] == true)
	{
		...
		{
			CPrintToChat(client, CHAT_BANNER, "Enforce Existing Mute");
		}	
	}
	...
	else if (DeadTalkMode == -2)
	{
		if (!GetConVarBool(gH_Cvar_TeamTalk))
		{
			...
			if (GetConVarInt(gH_Cvar_Announce))
			{
				CPrintToChat(client, CHAT_BANNER, "DeadAllTalk - Spawn - TeamTalk Off");
			}
		}
		else
		{
...
public Action:Timer_DoTeam(Handle:timer, any:client)
{
	if (IsClientInGame(client))
	{
		...
		if (GetConVarBool(gH_Cvar_Announce))
		{
			CPrintToChat(client, CHAT_BANNER, "DeadAllTalk - Spawn - TeamTalk On");
		}
	}
	return Plugin_Stop;
}
	...
	if (!g_BetweenRounds || (g_BetweenRounds && !RoundEndAllTalk))
	{
		if (DeadTalkMode == -1)
		{
			if (!GetAdminFlag(GetUserAdmin(client), Admin_Chat) || !AdminOverrideDeath)
			{
				...
				{
					CPrintToChat(client, CHAT_BANNER, "Muted On Death - Not Admin");
				}
			} // end if not admin
			else if (AdminOverrideDeath)
			{
				if (GetConVarBool(gH_Cvar_Announce) == bool:true)
				{
					CPrintToChat(client, CHAT_BANNER, "Muted On Death - Admin");
				}
			} // else if admin
		}
		else if ((DeadTalkMode == -2) && !GetConVarBool(gH_Cvar_AllTalk))
		{
			...
			if (GetConVarInt(gH_Cvar_Announce))
			{
				...
				{
					CPrintToChat(client, CHAT_BANNER, "DeadAllTalk - Death - Admin");
				}
				else
				{
					CPrintToChat(client, CHAT_BANNER, "DeadAllTalk - Death - Not Admin");
				}
			}
		}
...
public Action:RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (GetConVarBool(gH_Cvar_RoundEndAllTalk))
	{
		...
		if (GetConVarInt(gH_Cvar_Announce))
		{
			CPrintToChatAll(CHAT_BANNER, "Round End - All Talk On");
		}
	}
	else
	{
		if (GetConVarInt(gH_Cvar_Announce))
		{
			CPrintToChatAll(CHAT_BANNER, "Round End - All Talk Off");
		}
	}
	...
	if (g_bHasJoinedOnce[client] == bool:false)
	{
		...
		{	
			...
			{	
				...
				if (GetConVarBool(gH_Cvar_Announce))
				{
					CPrintToChat(client, CHAT_BANNER, "Mute On Join");
				}
			} // end if not an admin
			// else if admin
			else if (AdminOverrideJoin)
			{
				if (GetConVarBool(gH_Cvar_Announce))
				{
					CPrintToChat(client, CHAT_BANNER, "Admin On Join");
				}
			}
		} // end if deadtalkmode
	} // end if has joined
...
public Action:Listen_EffectiveMute(client, const String:command[], args)
{
	...
	{	
		...
		GetClientAuthId(target_list[i], AuthId_Steam2, sSteamID, sizeof(sSteamID));
	...
	for (new idx = 0; idx < iTimeArraySize; idx++)
	{
		...
		if (IsClientInGame(iClientIndex))
		{
			...
			if (gA_LocalTimeRemaining[iClientIndex] <= 0)
			{
				...
				decl String:sSteamID[22];
				GetClientAuthId(iClientIndex, AuthId_Steam2, sSteamID, sizeof(sSteamID));
