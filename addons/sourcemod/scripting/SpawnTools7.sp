...
{
	CreateConVar("sm_spawntools7_version", VERSION, "Spawn Tools 7 Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public MainMenuHandler(Handle:menu, MenuAction:action, client, selection)
{
	if (action == MenuAction_Select)
	{
		if (selection == 0)
		{
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} Edit Mode %s.", InEditMode == false ? "Disabled" : "Enabled");
			if (InEditMode)
		...
		else if (selection == 1)
		{
			...
			CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} Default Spawn Removal will be %s.", RemoveDefSpawns == false ? "Disabled" : "Enabled");
			ShowToolzMenu(client);
		}
		else if (selection == 2)
		{
		...
		else if (selection == 4)
		{
			if (!RemoveSpawn(client))
				CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} No valid spawn point found.");
			else
				CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} Spawn point removed!");
...
InitializeNewSpawn(client, team)
{
	...
	if (CreateSpawn(DataFloats, true))
		CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} New spawn point created!");
	else
...
SaveConfiguration()
{
	...
	if (KeyValuesToFile(kv, MapCfgPath))
		CPrintToChatAll("{chartreuse}[{dodgerblue}SpawnTool{chartreuse}]{default} Configuration Saved!");
	else
