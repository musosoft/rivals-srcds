...
#define PLUGIN_VERSION "2.5.2-private"
...
{
	CreateConVar("anticamp_css_version", PLUGIN_VERSION, "anticamp_css_version", FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public Action:CaughtCampingTimer(Handle:timer, any:client)
{
	...
	else
	{
		...
		// print to chat
		if(!Location)
			CPrintToChatAllEx(client, "{chartreuse}[{dodgerblue}AntiCamp{chartreuse}]{default} %t", "Player camping location", name, weapon, place);
		else
			CPrintToChatAllEx(client, "{chartreuse}[{dodgerblue}AntiCamp{chartreuse}]{default} %t", "Player camping", name, weapon);
