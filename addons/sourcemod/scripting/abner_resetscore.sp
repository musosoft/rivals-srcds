...
#define PLUGIN_VERSION "1.5-private"
...
{  
	...
	RegConsoleCmd("r", CommandResetScore);
...
public Action MSG(Handle timer, any client)
{
	if(IsValidClient(client))
		CPrintToChat(client, "{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Restored");
}
...
{                        
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Plugin Disabled");
	...
	// 	{
	// 		CPrintToChat(client, "{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Score 0");
	// 		return Plugin_Continue;
	// 	}
	// }
	...
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "No Money", cost);
		return Plugin_Continue;
	}
	...
	if(GetConVarInt(hPublic) == 1)
	{
		...
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Player Reset Red", name);
		}
		else if(GetClientTeam(client) == 3)
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Player Reset Blue", name);
		}
		else
		{
			CPrintToChatAll("{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "Player Reset Normal", name);
		}
	}
	else
	{
		CPrintToChat(client, "{chartreuse}[{dodgerblue}ResetScore{chartreuse}]{default} %t", "You Reset");
	}
	return Plugin_Continue;
}
