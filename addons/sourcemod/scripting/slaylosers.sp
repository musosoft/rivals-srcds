...
#define PLUGIN_VERSION "1.4b2-private"
...
public Plugin:myinfo = {
  name = "Slay Losers",
  author = "DarkEnergy - Ownz and Frezzy, muso.sk",
...
public OnPluginStart()
{
  ...
  CreateConVar("oc_slaylosers_version", PLUGIN_VERSION, "Slay Losers version", FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
public Action:SlayTeam(Handle:t, any:team)
{
  if(bCanSlay) //avoid slaying right after round starts, if bomb explodes 0.1 seconds before a new round
  {
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
      if (IsClientConnected(i) && IsClientInGame(i) && IsPlayerAlive(i) && GetClientTeam(i) == team && !IsAdminImmunity(i) && !isFighting)
      {
...
public SlayLosersPrintToChat(const String:szMessage[], any:...)
{
  ...
  for (new i = 1; i <= MaxClients; i++)
  {
    if (IsClientInGame(i) && !IsFakeClient(i))
    {
      ...
      Format(szBuffer, sizeof(szBuffer), "%T%s", "[AutoSlay]", i, szBuffer);
