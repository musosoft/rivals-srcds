...
#define PLUGIN_VERSION "1.4-private"
...
public OnPluginStart()
{
	...
	CreateConVar("sm_vbping_version", PLUGIN_VERSION, "Very Basic High Ping Kicker Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	// Config Cvars
	cvar_MinTime = CreateConVar("sm_vbping_mintime",				"90",				"Minimum playtime before ping check begins");
	cvar_MaxPing = CreateConVar("sm_vbping_maxping",				"120",				"Maximum player ping", 0, true, 1.0);
	cvar_CheckRate = CreateConVar("sm_vbping_checkrate",			"20.0",				"Period in seconds when rate is checked", 0, true, 1.0);
	cvar_MaxWarnings = CreateConVar("sm_vbping_maxwarnings",		"4",				"Number of warnings before kick", 0, true, 1.0);
	cvar_MinPlayers = CreateConVar("sm_vbping_minplayers",			"8",				"Minimum number of players before kicking");
	cvar_LogActions = CreateConVar("sm_vbping_logactions",			"0",				"Log warning and kick actions. 0 = Disabled, 1 = Enabled", 0, true, 0.0, true, 1.0);
	cvar_ShowWarnings = CreateConVar("sm_vbping_showwarnings",		"1",				"Enable/disable warning messages. 0 = Disabled, 1 = Enabled", 0, true, 0.0, true, 1.0);
	cvar_WarningMsg = CreateConVar("sm_vbping_warningmsg",			"You will be kicked for excessive ping. You have {WARN} out of {MAXWARN} warnings.",				"Warning message sent to high ping client. {WARN} and {MAXWARN} converts to the warning count and max warnings respectively.");
	cvar_KickMsg = CreateConVar("sm_vbping_kickmsg",				"You have been kicked due to excessive ping",				"Kick message sent to kicked client");
	cvar_ShowPublicKick = CreateConVar("sm_vbping_showpublickick",	"0",				"Enable/disable public kick message. 0 = Disabled, 1 = Enabled", 0, true, 0.0, true, 1.0);
	cvar_KickMsgPublic = CreateConVar("sm_vbping_kickmsgpublic",	"{NAME} was been kicked due to excessive ping",				"Public kick message. {NAME} converts to the player's name.");
	cvar_ImmunityFlag = CreateConVar("sm_vbping_immunityflag",		"a",				"SourceMod admin flag used to grant immunity to all ping checking/kicking");
		...
		if (IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			if (PingWarnings[i] >= GetConVarInt(cvar_MaxWarnings))
			{
				...
				if (GetConVarBool(cvar_ShowPublicKick))
				{
					...
					CPrintToChatAll("{chartreuse}[{dodgerblue}HighPing{chartreuse}]{default} %s", Message);
				}
			...
			if (Ping > GetConVarInt(cvar_MaxPing))
			{
				...
				if (GetConVarBool(cvar_ShowWarnings))
				{
					...
					CPrintToChat(i, "{chartreuse}[{dodgerblue}HighPing{chartreuse}]{default} %s", Message);
				}
